...
function create_start_invoke_file() {
   ...
   echo "   sudo -i -u hadoop ${TAJO_HOME}/bin/tajo-daemon.sh start master" >> ${TAJO_HOME}/$1
   ...
   echo "      nc -z $TAJO_MASTER 26001" >> ${TAJO_HOME}/$1
   echo "   done" >> ${TAJO_HOME}/$1
   echo "   sudo -i -u hadoop ${TAJO_HOME}/bin/tajo-daemon.sh start worker" >> ${TAJO_HOME}/$1
   echo "fi" >> ${TAJO_HOME}/$1
   chmod +x ${TAJO_HOME}/$1
}
...
# Set ulimit: open file count
set -e -x
sudo sed -i -e "/^hadoop soft nofile/s|.*|* soft nofile 100000|" /etc/security/limits.conf
sudo sed -i -e "/^hadoop hard nofile/s|.*|* hard nofile 100000|" /etc/security/limits.conf
sudo sed -i -e "/^ulimit /s|.*|ulimit -n 100000|" /usr/lib/hadoop/hadoop-daemon.sh
echo "hadoop - nofile 100000" >> ${TAJO_HOME}/zz_tajo.conf
chmod 755 $TAJO_HOME/zz_tajo.conf 
sudo mv $TAJO_HOME/zz_tajo.conf /etc/security/limits.d/
